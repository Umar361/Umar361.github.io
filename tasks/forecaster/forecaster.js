...
function createChart(year) {
  ...
  chartDataset.push({
    ...
    data: currentData.map(function (row) {
      return row["solid"];
    }), // data in that column
...
document.getElementById("previous").addEventListener("click", function () {
  if (currentYear == 1996) {
    document.querySelector(".result_span").innerHTML = `No data available for ${
      currentYear - 1
    }`;
    return false;
  } else {
...
function populateTable() {
  ...
  solidDiff = (solidMax - solidMin) / solidMax;
  liquidDiff = (liquidMax - liquidMin) / liquidMax;
  gasDiff = (gasMax - gasMin) / gasMax;
  elecDiff = (elecMax - elecMin) / elecMax;
  for (var i = 0; i < 12; i++) {
    var td = document.createElement("td");
    td.innerHTML = (lastyear[i].solid + lastyear[i].solid * solidDiff).toFixed(
      2
    );
    ...
    td.innerHTML = (
      lastyear[i].liquid +
      lastyear[i].liquid * liquidDiff
    ).toFixed(2);
    trLiquid.appendChild(td);
    td = document.createElement("td");
    td.innerHTML = (lastyear[i].gas + lastyear[i].gas * gasDiff).toFixed(2);
    trGas.appendChild(td);
    td = document.createElement("td");
    td.innerHTML = (
      lastyear[i].eletric +
      lastyear[i].eletric * elecDiff
    ).toFixed(2);
