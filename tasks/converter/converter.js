...
const btnFunc = () => {
    ...
    if (realAmount === '' ) {
        ...
        result_par.innerText = `Please select ${ converTo === base ? 'currencies ' : base === "select" ? 'source currency': 'destination currency'  } for conversion`
    }
        ...
        <h6 class="result_par">${realAmount} ${fromCurrency.code} is equal to <span class="result_span">${convert().toLocaleString()} ${toCurrency.code}</span></h6> 
        <h6 class="result_par">Conversion Rate: <span class="result_span">${toCurrency.rate}</span></h6>
